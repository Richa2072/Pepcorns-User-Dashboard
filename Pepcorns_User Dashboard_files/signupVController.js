app.controller('signupVController', ['$scope','$state','$http','apiService','authService','$compile','$stateParams','$window','$location','helperService','CONSTANT',function($scope,$state,$http,apiService,authService,$compile,$stateParams,$window,$location,helperService,CONSTANT){
    var self = $scope;
    self.verifyEmail={};
    self.pagenum = 1;
    ...
    //self.email_data.email="gg@gg.com";
    //self.email_data.country_code="91";
    console.log(self.min_signup_date)
    // console.log($location.path())
    // //console.log($location.search('session_id'))
    // var session_set = $location.path().split('=');
    // session_set.shift();
    // console.log(session_set)
    // self.session_id= session_set.join();
    ...
    self.verifySession = function(){
        ...
        if(self.session_id == undefined){
            ...
            apiService.verifySessionSignup({"session_key":self.session_id }).then(function (data) {
                //self.setPageNum(1);
    ...
    //         return
    //     }
    //     else if(!helperService.validateName(self.email_data.first_name)){
    ...
    //     if(self.email_data.pass.length < 6 || self.email_data.pass1.length < 6){
    //         createAlertErr("ni ni-fat-remove", "Password should be minimum 6 and maximum 20 characters long ");
    //         return
    //     }
    //     else if(self.email_data.pass != self.email_data.pass1){
    //         createAlertErr("ni ni-fat-remove", "Password and Confirm password did not match.");
    //         return
    //     }
    //     else{
    //         self.showLoader=true;
